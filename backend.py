...
def video_loop():
    ...
    try:
        ...
        # 项目A行为识别引擎初始化（21种行为 + XGBoost + MediaPipe）
        behavior_detector = None
        if BEHAVIOR_ENGINE_AVAILABLE:
            try:
                behavior_detector = VideoBehaviorDetector()
                print("VideoBehaviorDetector loaded - 21 behavior types enabled in live feeds.")
            except Exception as e:
                ...
                if cap.isOpened() and 'ret' in locals() and ret:
                    ...
                    if results_pose[0].boxes.id is not None:
                        ...
                        for i, track_id in enumerate(track_ids):
                            ...
                            # --- 项目A行为识别引擎（21种行为 + XGBoost + MediaPipe） ---
                            # 节流：每15帧或出现可疑动作时对当前人物做精细分析
                            if (behavior_detector is not None) and (frame_count % 15 == 0 or is_bending):
                                try:
                                    pad = 60
                                    sx1 = max(0, box[0] - pad)
                                    sy1 = max(0, box[1] - pad)
                                    ...
                                    sub_behaviors = behavior_detector.detect_behaviors_in_image(sub_frame, None)
                                    for b in sub_behaviors:
                                        btype = str(b.get("type", "Unknown"))
                                        bconf = float(b.get("confidence", 0))

                                        # 帧上标注行为类型 + 置信度
                                        label = f"{btype}: {bconf:.2f}"
                                        cv2.putText(frame, label, (box[0], box[3] + 25),
                                                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)

                                        # 高置信度行为触发报警 + 写入SQLite历史
                                        if bconf >= 0.7 and (current_time - cam_data["last_alert_time"] > ALERT_COOLDOWN):
                                            trigger_alert(cam_id, name, f"BEHAVIOR: {btype} ({bconf:.2f})", frame)
                                            cam_data["last_alert_time"] = current_time
                                except Exception as e:
                                    print(f"Behavior engine error (track {track_id}): {e}")
...
async def analyze_image(file: UploadFile = File(...)):
    """分析上传的图片，返回21种行为检测结果"""
    if not BEHAVIOR_ENGINE_AVAILABLE:
        raise HTTPException(status_code=503, detail="Behavior engine not available")
    ...
    try:
        ...
        detector, img_behavior = get_behavior_detectors()

        # 执行物体检测 + 行为分析
        detection_output, detection_results = detector.process_image(temp_path)
        behavior_output, behaviors = img_behavior.analyze_image(temp_path, detector)
        ...
        # 结果图转 base64（方便前端直接显示）
        result_image_b64 = None
        result_path = behavior_output if behavior_output else detection_output
        ...
        return {
            ...
            "behaviors_count": len(serializable_behaviors),
...
async def analyze_video(file: UploadFile = File(...)):
    ...
    try:
        ...
        detector, _ = get_behavior_detectors()
        video_behavior = VideoBehaviorDetector()

        output_path, suspicious_frames, behaviors = video_behavior.analyze_video_behavior(temp_path, detector)
        ...
        for frame_idx, frame_behaviors in behaviors:
            for b in frame_behaviors:
                behavior_data.append({
                    "frame": int(frame_idx),
                    "time": float(frame_idx / 30.0),
                    "type": str(b.get("type", "Unknown")),
                    "description": str(b.get("description", "")),
                    "confidence": float(b.get("confidence", 0))
                })

        # 结果视频（小视频转 base64，大视频建议走文件URL）
        result_video_b64 = None
        if output_path and os.path.exists(output_path):
            if os.path.getsize(output_path) < 5 * 1024 * 1024:
                with open(output_path, "rb") as f:
                    result_video_b64 = base64.b64encode(f.read()).decode("utf-8")

        return {
            "behaviors": behavior_data,
            "behaviors_count": len(behavior_data),
            "suspicious_frames": [int(f) for f in suspicious_frames] if suspicious_frames else [],
            "result_video": result_video_b64,
            "type": "video"
        }
    except HTTPException:
        raise
