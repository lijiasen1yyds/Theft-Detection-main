                      ...
                      if (m) {
                        ...
                        return (
                          <span className={`flex items-center gap-1.5 text-xs font-bold animate-pulse px-2 py-0.5 rounded ${cls}`}>
                            ...
                            <span>{btype}</span>
                            <span className="px-1.5 py-0.5 bg-black/50 rounded text-[10px] font-mono">{pct}%</span>
                          </span>
                        );
                      }
                      // 非行为alert（如ROI闯入、徘徊、黑名单人脸）走原逻辑
                      return (
                        <span className="flex items-center gap-1 text-xs text-danger font-bold animate-pulse bg-danger/20 px-2 py-0.5 rounded">
                          <AlertTriangle className="w-3 h-3" />
                          {alertMessage}
                        </span>
                      );
